...
class AirCargoProblem(Problem):
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        '''
        '''
        If there are no preconditions then the cost of cargo problem is length of goal expressions
        If the current node has any of the goal expressions then count is count-1
        '''
        count = len(self.goal)
        for expr in decode_state(node.state, self.state_map).pos:
...
if __name__ == '__main__':
    ...
    run_search(p1, greedy_best_first_graph_search, parameter=p1.h_ignore_preconditions)
    print("*** A-star null heuristic")
    run_search(p1, astar_search, p.h_1)
    run_search(p1, greedy_best_first_graph_search, parameter=p1.h_pg_levelsum)