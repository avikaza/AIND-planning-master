...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        '''
        # Get all positive symbols and negative symbols at s[level]
        s_pos_symbols = set()
        s_neg_symbols = set()
        for s in self.s_levels[level]:
            if s.is_pos:
                s_pos_symbols.add(s.symbol)
            else:
                s_neg_symbols.add(s.symbol)
        
        self.a_levels.append(set())
        #Check if each action's preconditions are met in s_level and add to a_level
        for action in self.all_actions:
            if s_pos_symbols.issuperset(action.precond_pos) and s_neg_symbols.issuperset(action.precond_neg):
                good_a_node = PgNode_a(action)
                #add all nodes from s_level as parents and this one as their child
                for s in self.s_levels[level]:
                    good_a_node.parents.add(s)
                    s.children.add(good_a_node)
                
                self.a_levels[level].add(good_a_node)
    ...
    def add_literal_level(self, level):
        ...
        # Get previous level A nodes and convert their effects into new s node (sets are awesome!)
        for prev_a_node in self.a_levels[level-1]:
            for effect_node in prev_a_node.effnodes:
                new_s_node = effect_node
                prev_a_node.children.add(new_s_node)
                new_s_node.parents.add(prev_a_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        # The pair's effects should have atleast one common symbol but one must be positive and other negative
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        #If this action's effect_rem is in precondition_pos of other node then return True
        for e_rem in node_a1.action.effect_rem:
            if e_rem in node_a2.action.precond_pos:
                return True
        for e_rem in node_a2.action.effect_rem:
            if e_rem in node_a1.action.precond_pos:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        # If 2 childs are with competing needs then their parents must be mutex of each other
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        '''
        # Nodes directly opposing each other (Eaten(cake) vs NotEaten(cake))
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        '''
        # If two states whose literals cannot be achieved by any actions then return true
        # If two parents are not mutually exclusive then their children are not mutually exclusive 
        # States are mostly mutually exclusive
    ...
    def h_levelsum(self):
        ...
        # Make sure we do not double count a goal in multiple levels
        for goal in self.problem.goal:
            goal_reached = False
            for cur_level in range(len(self.s_levels)):
                for s in self.s_levels[cur_level]:
                    if s.literal == goal:
                        level_sum = level_sum + cur_level
                        goal_reached = True
                        break
                if goal_reached:
                    break
